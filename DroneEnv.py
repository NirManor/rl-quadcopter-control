...
class DroneEnv(gymnasium.Env):
    def __init__(self, reward_type):
        ...
        self.action_scaling = 3.18825  # (0.12+0.1*4)*2.5*9.81/4 = 3.18825
    ...
    def step(self, action):
        ...
        print(f"Orientation: {np.rad2deg(drone_ori)}")
        print(f"Velocities - Linear: {drone_v_lin}")
        print(f"Velocities - Angular: {drone_v_ang}")
        print(f"Step reward: {reward}")
        print(f"Total time steps: {self.total_timesteps}")
        print("\n")
    ...
    def calculate_reward(self, terminated, termination_reason):
        ...
        if self.reward_type == "stabilize":
            reward = r_stabilize
            if terminated and termination_reason in "angle_too_big":
                reward -= 100
    ...
    def reset(self, seed=None):
        ...
        initial_target_pos = (-0.55 , 0.6 , 0.51 )
        ...
        sim.simxSetObjectOrientation(self.drone_sim_model.client_ID, self.drone_sim_model.heli_handle, -1, initial_ori,
                                     opmode)
        # sim.simxSetObjectPosition(self.drone_sim_model.client_ID, self.drone_sim_model.target_handle, -1,
        #                           initial_target_pos,
        #                           opmode)
